...
//  base_ang_nodes->AddObserver(base_angular_);
  ...
  for (uint ee=0; ee<ee_motion_nodes.size(); ++ee) {
    ...
    auto ee_spline = std::make_shared<Spline>(ee_motion_nodes.at(ee), contact_schedule.at(ee));
    ee_motion_.push_back(ee_spline);
//    ee_motion_nodes.at(ee)->AddObserver(ee_spline);

    auto ee_force_spline = std::make_shared<Spline>(ee_force_nodes.at(ee), contact_schedule.at(ee));
