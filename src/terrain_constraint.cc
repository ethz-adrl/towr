                                              ...
                                              Jacobian& jac) const
{
  if (var_set == ee_motion_->GetName()) {
    ...
    for (int i=0; i<nodes.size(); ++i) {
      ...
      Vector3d p = nodes.at(i).at(kPos);
      jac.coeffRef(i, ee_motion_->Index(i, kPos, X)) = -terrain_->GetHeightFirstDerivativeWrt(X, p.x(), p.y());
      jac.coeffRef(i, ee_motion_->Index(i, kPos, Y)) = -terrain_->GetHeightFirstDerivativeWrt(Y, p.x(), p.y());
    }
  }
}
