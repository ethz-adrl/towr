    ...
    double t_stance_final)
{
  ...
  splines.push_back(ZmpSpline(id++, t_stance_initial, true, step));

  int n_steps = step_sequence.size();
  if (n_steps > 0)
    splines.push_back(ZmpSpline(id++, t_swing, false, step));
  ...
  for (int i=1; i<n_steps; ++i)
  {
    ...
    if (xpp::hyq::SupportPolygonContainer::Insert4LSPhase(swing_leg_prev, swing_leg))
      splines.push_back(ZmpSpline(id++, t_stance, true, step));
  ...
  if (n_steps > 0)
    splines.push_back(ZmpSpline(id++, t_stance_final, true, step+1));
